...
export default function Deals() {
  ...
  useEffect(() => {
    if (!mapsReady || !geoLocation) return;
    loadNearby(activeQuery.query);
  }, [mapsReady, geoLocation, activeQuery]);

  const loadNearby = async (query: string) => {
    if (!geoLocation || !mapsReady) return;
    setNearbyLoading(true);
    setNearbyError('');
    try {
      const results = await searchNearbyPlaces(geoLocation, query);
      setNearby(results);
    ...
    <>
      ...
      <div style={{ display: 'flex', gap: 24, padding: '24px 32px 48px', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: '1 1 460px', minWidth: 0, display: 'flex', flexDirection: 'column', gap: 32 }}>
          ...
          <div>
            ...
            {!env.googlePlacesKey ? (
              <div style={{ padding: '14px 16px', background: '#fff8e6', border: '1px solid #f0d090', borderRadius: 10, fontSize: 13 }}>
                ⚠️ Add <code>VITE_GOOGLE_PLACES_API_KEY</code> to <code>.env.local</code> to show nearby businesses automatically.
              </div>
            ) : !geoLocation ? (
          ...
          {/* Section 2: Community-posted Deals */}
          <div>
