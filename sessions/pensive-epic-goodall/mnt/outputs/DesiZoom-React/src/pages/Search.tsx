...
export default function Search() {
  ...
  const doSearch = async (categoryQuery?: string) => {
    if (!geoLocation) { setError('Location not available yet.'); return; }
    if (!mapsReady) { setError('Maps API not ready. Add VITE_GOOGLE_PLACES_API_KEY to .env.local'); return; }
    setLoading(true);
    setError('');
    try {
      const q = categoryQuery || query || activeCategory.query;
      const data = await searchNearbyPlaces(geoLocation, q);
      setResults(data);
  ...
  useEffect(() => {
    if (mapsReady && geoLocation) doSearch(activeCategory.query);
  }, [mapsReady, geoLocation, activeCategory]);
