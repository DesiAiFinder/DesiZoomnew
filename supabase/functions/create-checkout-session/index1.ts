...
// Commission: 5% + $0.50 flat
function calcCommission(priceCents: number): number {
  return Math.round(priceCents * 0.05) + 50;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!post.stripe_account_id) throw new Error('Seller has not connected their bank account yet');
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      payment_intent_data: {
        ...
        transfer_data: {
          destination: post.stripe_account_id,
        },
      },
      metadata: {
        post_id: post.id,
      ...
      success_url: success_url || 'https://desizoom.com/marketplace?payment=success',
      cancel_url: cancel_url || 'https://desizoom.com/marketplace?payment=cancelled',
    });
    ...
    await supabase.from('payments').insert({
      ...
      stripe_account_id: post.stripe_account_id,
      status: 'pending',
    });
